  ...
  public:
    ...
    static std::vector<cv::Point2f> getPersonPoints(std::string source, cv::Mat tenSecDiff) {
      ...
      cv::Mat image = cv::imread(
                cv::samples::findFile("test4.jpg"), cv::IMREAD_COLOR);
      cv::subtract(image, tenSecDiff, image);
      ...
      cv::HoughCircles(image, circles, cv::HOUGH_GRADIENT, 1, image.rows/10, 150, 20, 20, 100);
      ...
      for( size_t i = 0; i < circles.size(); i++ )
      {
          ...
          // circle outline
          cv::circle(image, center, radius, cv::Scalar(0,0,0), 2, 8, 0 );
      }
...
int main() {
  cv::Mat tenSecDiff = cv::imread(
                cv::samples::findFile("test3.jpg"), cv::IMREAD_COLOR);
